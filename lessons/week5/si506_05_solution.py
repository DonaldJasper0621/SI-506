...
def main():
    ...
    i = 0 # reset
    while True:
        city = get_element(stations[i], headers, "city")
        if city.strip().lower() == "ypsilanti":
            ypsi_station_idx = stations.index(stations[i])
            break  # exit loop
        i += 1  # increment
    ...
    # Alternative
    ypsi_station_idx = None
    for station in stations:
        city = get_element(station, headers, "city")
        if city.strip().lower() == "ypsilanti":
            ypsi_station_idx = stations.index(station)
            break  # exit loop

    print(f"\n5.2.2 First Ypsi EV station index val = {ypsi_station_idx}")
    assert ypsi_station_idx == 18
    ...
    for station in stations:
        if (
            station[name_idx].strip().startswith("U-M")
            and "greene st" in station[street_idx].lower()
        ):
    ...
    for i in range(len(stations)):
        if (
            stations[i][name_idx].strip().startswith("U-M")
            and "greene st" in stations[i][street_idx].lower()
        ):
            um_stations.append(stations[i])
        i += 1

    print(f"\n7.2.2 U-M Greene St stations (n={len(um_stations)})")
    pp.pprint(um_stations)
    ...
    print(f"\n7.2.3 U-M Greene St stations (n={len(um_stations)})")
    pp.pprint(um_stations)
    ...
    print(f"\n7.2.4 U-M Greene St stations (n={len(um_stations)})")
    pp.pprint(um_stations)
    ...
    a2dda_stations = []
    for station in stations:
        name = get_element(station, headers, "station_name").strip()  # Don't lowercase
        street = get_element(station, headers, "street_address").strip()  # Don't lowercase
        if (
            name.startswith("A2DDA") or name.startswith("Ann Arbor Downtown Development Authority")
        ) and ("forest" in street.lower() or "maynard" in street.lower()):
            a2dda_stations.append(f"{name} {street}")
