...
def count_vehicles_by_range(vehicles, headers, range_mi=250):
    ...
    for vehicle in vehicles:
        ...
        if int(vehicle[range_mi_idx]) >= range_mi:  # cast str to int
            vehicle_count += 1  # equivalent to vehicle_count = vehicle_count + 1
...
def get_make_model(vehicle):
    return f"{vehicle[0]} {vehicle[1]}"


def get_vehicles(vehicles, automaker):
    autos = []
    for vehicle in vehicles:
        if automaker.lower() in vehicle[0].lower():
            # if vehicle[0].lower().find(automaker.lower()) > -1:
            # if vehicle[0].lower().startswith(automaker.lower()):
...
def main():
    ...
    # SETUP (create list of strings (first 5 vehicles))
    vehicles = [", ".join(vehicle) for vehicle in E_VEHICLES[1:6]]
    print("\n3.0 Vehicles (first 5)")
    pp.pprint(vehicles)
    ...
    for vehicle in E_VEHICLES[1:]:
        if vehicle[0].find("Kia") > -1:
            # if vehicle[0].startswith("Kia"):  # alternative
            print(vehicle)
    ...
    headers = E_VEHICLES[0]  # column headers
    model_idx = headers.index("model")  # lookup index
    print(f"\n3.4.1.1 model index = {model_idx}")

    vehicles = E_VEHICLES[1:]  # access nested lists
    ...
    range_mi_idx = headers.index("range_mi")  # lookup index
    lucid_range_mi = lucid[range_mi_idx]

    # Alternative
    lucid_range_mi = lucid[headers.index("range_mi")]  # one line

    # TypeError: 'list' object is not callable
    # lucid_range_mi = lucid(headers.index("range_mi"))  # Use [...] not (...)
    ...
    make_model = get_make_model(vehicles[-2])
    ...
    rivian_models = get_vehicles(vehicles, "Rivian")
    print(f"\n4.1.1 Rivian EVs (n={len(rivian_models)})")
    pp.pprint(rivian_models)

    # 4.1.3 CHALLENGE 04

    ev_min_range = [None, 10000]  # store in list
    # ev_min_range = [None, float("inf")] # infinity
    for vehicle in vehicles:
        range_mi_idx = headers.index("range_mi")  # lookup index
        vehicle_range = int(vehicle[range_mi_idx])
    ...
    vehicle_count = count_vehicles_by_range(vehicles, headers)
    ...
    foreign_count = 0
    for vehicle in vehicles:
        if is_domestic(vehicle, us_automakers):
            domestic_count += 1
        else:
            foreign_count += 1
    ...
    for vehicle in vehicles:
        ...
        msrp = int(msrp[1:].replace(",", ""))  # slice excludes "$"
        make_model = get_make_model(vehicle)
        if msrp <= 45000:
            affordable.append(make_model)
        else:
            unaffordable.append(make_model)
    ...
    for vehicle in vehicles:
        ...
        range_mi = int(vehicle[headers.index("range_mi")])
        if drivetrain == "AWD":
            awd_range.append(range_mi)
        elif drivetrain == "FWD":
            fwd_range.append(range_mi)
        elif drivetrain == "RWD":
            rwd_range.append(range_mi)

    awd_avg_range = sum(awd_range) / len(awd_range)
    fwd_avg_range = sum(fwd_range) / len(fwd_range)
    rwd_avg_range = sum(rwd_range) / len(rwd_range)
    ...
    for i in range(0, len(automakers), 3):
        automakers[i] = automakers[i].lower()  # assign new string elements at positions 0, 3, 6
    ...
    for i in range(len(vehicles)):
        make_model = get_make_model(vehicles[i])
        if vehicles[i][headers.index("type")] == "Sedan/Wagon":
            sedan_wagon.append(make_model)
        else:
            suv_pickup.append(make_model)
    ...
    ev_types = get_ev_types(vehicles)
    ...
    for i in range(len(vehicles)):
        if vehicles[i][2].lower() == "suv":
            vehicles[i][2] = "Sports Utility Vehicle"  # rename
